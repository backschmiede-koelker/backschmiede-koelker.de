...
async function fetchOffers() {
  ...
  const host = h.get("x-forwarded-host") ?? h.get("host") ?? process.env.VERCEL_URL ?? "192.168.178.163:3000";
  const proto = h.get("x-forwarded-proto") ?? (process.env.VERCEL_URL ? "https" : "http");
  const url = `${proto}://${host}/api/offers?type=weekly`;
