...
function usePerIndexEqualHeights(
  ...
  React.useLayoutEffect(() => {
    ...
    const measure = () => {
      ...
      // 1) Reset heights vor der Messung (verhindert “aufgeblasene” Werte)
      ...
      // 3) exakte Höhe anwenden (nur ≥lg), <lg wieder auto
      for (let i = 0; i < maxLen; i++) {
        const L = leftRefs.current[i];
        const R = rightRefs.current[i];
        const h = next[i];
    ...
    const schedule = () => {
      ...
      raf = requestAnimationFrame(() => {
        // ein weiteres rAF hilft bei Fonts/Wraps
    ...
    // Änderungen an den Zeilen beobachten (Wraps, Contentänderung)
...
function WeekTable({
  ...
  liRefs: React.MutableRefObject<(HTMLLIElement | null)[]>;
    ...
    const setRowRef = (idx: number) => (el: HTMLLIElement | null) => { liRefs.current[idx] = el; };
    ...
    return (
        ...
        {lines.map((line, i) => {
            ...
            return (
            <li ref={setRowRef(i)} key={`${i}-${day}`} className="py-1">
                <div className="relative">
                {isToday && <span aria-hidden className="absolute inset-0 rounded-md bg-emerald-50/70 dark:bg-emerald-900/20" />}

                <div className={[
                    "relative grid items-center text-sm leading-5",
                    "grid-cols-2 gap-x-2",                              // <1024px: 50/50
                    "lg:grid-cols-[9.25rem,minmax(0,1fr)] xl:grid-cols-[10rem,minmax(0,1fr)]" // ≥1024px fix/auto
                ].join(' ')}>
