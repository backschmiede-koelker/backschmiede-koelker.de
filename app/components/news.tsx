...
type ApiNews = {
  ...
  title: string;
  body: string;
  imageUrl?: string | null; // API liefert absolute URL oder Stored-Path; publicAssetUrl handhabt beides
  tag?: string | null;
...
function TagBadge({ tag }: { tag?: string | null }) {
  ...
  const base = "inline-flex items-center rounded-full px-2 py-0.5 text-[11px] font-semibold uppercase tracking-wide ring-1 ring-inset";
  let styles = "bg-slate-500/20 dark:bg-slate-300/20 text-slate-900 dark:text-slate-100 ring-slate-600/35 dark:ring-slate-400/35";
...
export default async function News() {
  const res = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL ?? ""}/api/news?active=1&limit=4`, { cache: "no-store" });
          ...
          >
            {img && (
              <div className="relative aspect-[16/9] w-full">
                <Image src={img} alt={n.title} fill className="object-cover" sizes="(max-width: 640px) 100vw, 50vw" />
              </div>
            )}
            ...
            <div className="p-4 sm:p-5">
              ...
              <p className="mt-2 whitespace-pre-line text-sm text-zinc-700 dark:text-zinc-300">{n.body}</p>
