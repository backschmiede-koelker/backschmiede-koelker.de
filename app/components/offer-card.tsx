...
export default function OfferCard({ offer: o, allUnits, onSaved, onDelete }: Props) {
  ...
  const [imageUrl, setImageUrl] = useState(o.imageUrl ?? "");
  ...
  }, [title, slug, description, price, unitFinal, imageUrl, tags, isActive, kind, weekday, date, startDate, endDate, locations, priority, o]);
  ...
  async function save() {
    setSaving(true);
    try {
      if (priceInvalid) { alert("Ungültiger Preis"); return; }
      const body: any = {
        title: title.trim(),
    ...
    <li className="rounded-2xl border bg-white/90 p-4 ring-1 ring-black/5 shadow-sm dark:bg-zinc-900/80 dark:ring-white/10">
      ...
      <div className="grid grid-cols-[56px,1fr,auto] gap-3 sm:grid-cols-[56px,1fr,auto]">
        ...
        <div className="min-w-0">
          <div className="flex flex-col gap-1 sm:flex-row sm:items-baseline sm:justify-between">
            ...
            <div className="text-xs text-zinc-500 sm:ml-3">{slug || "—"}</div>
          </div>
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-6">
        ...
        <div className="md:col-span-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-end">
          <button
            onClick={save}
            disabled={!changed || priceInvalid || !!saving}
