// app/api/about/values/[id]/route.ts
...
export async function PUT(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const data: Partial<{ title: string; description: string | null; sortOrder: number }> = {};
  ...
  const updated = await getPrisma().aboutValueItem.update({ where: { id }, data });
...
export async function DELETE(_: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  try {
    await getPrisma().aboutValueItem.delete({ where: { id } });
