...
function toDTO(item: any) {
  ...
  if (item.type === "PRODUCT_NEW") {
    return { ...common, productNew: item.productNew ? { product: item.productNew.product, highlightLabel: item.productNew.highlightLabel ?? null } : undefined };
  }
  if (item.type === "PRODUCT_DISCOUNT") {
    return { ...common, productDiscount: item.productDiscount ? { product: item.productDiscount.product, priceCents: item.productDiscount.priceCents, originalPriceCents: item.productDiscount.originalPriceCents ?? null, unit: item.productDiscount.unit ?? null } : undefined };
  }
  if (item.type === "MULTIBUY_PRICE") {
    return { ...common, multibuyPrice: item.multibuyPrice ? { product: item.multibuyPrice.product, packQty: item.multibuyPrice.packQty, packPriceCents: item.multibuyPrice.packPriceCents, comparePackQty: item.multibuyPrice.comparePackQty ?? null, comparePriceCents: item.multibuyPrice.comparePriceCents ?? null, unit: item.multibuyPrice.unit ?? null } : undefined };
  }
  return common;
}
...
export async function PUT(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const current = await prisma.offer.findUnique({
      where: { id },
      include: { generic: true, productNew: true, productDiscount: true, multibuyPrice: true },
    });
    ...
    data.weekday   = data.kind === "RECURRING_WEEKDAY" ? (b.base?.weekday ?? null) : null;
    data.date      = data.kind === "ONE_DAY" ? (b.base?.date ? startOfDay(new Date(`${b.base.date}T00:00:00.000Z`)) : null) : null;
    data.startDate = data.kind === "DATE_RANGE" ? (b.base?.startDate ? startOfDay(new Date(`${b.base.startDate}T00:00:00.000Z`)) : null) : null;
    data.endDate   = data.kind === "DATE_RANGE" ? (b.base?.endDate ? endOfDay(new Date(`${b.base.endDate}T00:00:00.000Z`)) : null) : null;
    ...
    if (b.base?.minBasketCents     !== undefined) data.minBasketCents     = b.base.minBasketCents == null ? null : Number(b.base.minBasketCents);
    if (b.base?.priceCents         !== undefined) data.priceCents         = b.base.priceCents == null ? null : Number(b.base.priceCents);
    if (b.base?.originalPriceCents !== undefined) data.originalPriceCents = b.base.originalPriceCents == null ? null : Number(b.base.originalPriceCents);
    ...
    if (current.type === "PRODUCT_DISCOUNT" && b.payload) {
      await prisma.offerProductDiscount.update({
        ...
        data: {
          productId: b.payload.productId ?? undefined,
          priceCents: b.payload.priceCents != null ? Number(b.payload.priceCents) : undefined,
          originalPriceCents: b.payload.originalPriceCents == null ? null : Number(b.payload.originalPriceCents),
    ...
    if (current.type === "MULTIBUY_PRICE" && b.payload) {
      await prisma.offerMultibuyPrice.update({
        ...
        data: {
          productId: b.payload.productId ?? undefined,
          packQty: b.payload.packQty != null ? Math.max(1, Number(b.payload.packQty)) : undefined,
          packPriceCents: b.payload.packPriceCents != null ? Number(b.payload.packPriceCents) : undefined,
          comparePackQty: b.payload.comparePackQty == null ? null : Math.max(1, Number(b.payload.comparePackQty)),
          comparePriceCents: b.payload.comparePriceCents == null ? null : Number(b.payload.comparePriceCents),
          unit: b.payload.unit ?? null,
        },
      });
    }
...
export async function DELETE(_: Request, ctx: { params: Promise<{ id: string }> }) {
  const { id } = await ctx.params;
  try {
    const prev = await prisma.offer.findUnique({ where: { id }, select: { imageUrl: true } });
